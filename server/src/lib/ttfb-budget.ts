...
// Simple key construction: avoid circular dep on endpoint-health.ts by
// duplicating the identity logic here (it's just a string concat).
function endpointHealthKey(platform: string, endpointScope: string): string {
...
/** Called off the hot path after each successful request. */
export function recordTtfbSample(platform: string, endpointScope: string, ttfbMs: number, nowMs = Date.now()): void {
  ...
  rec.cachedBudgetMs = 0; // invalidate cache
}
 ...
 * Returns the budget (never null — falls back to base when no data).
 */
